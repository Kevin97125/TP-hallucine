...
class HallucineModel extends Model{
    ...
    public function requestMovie(int $movie){
        $sql = "SELECT * FROM `movies` WHERE id = $movieId;";
        $rows = $this->getRows(HOST, DB_NAME, LOGIN, PASSWORD, $sql);
        $value = $rows[0];
        $movie = new Movie($value["id"], $value["title"], $value["image_url"], $value["runtime"], $value["description"], $value["release_date"], $value["added_date"]);
        $this->_movie = $movie;
    }

    public function requestMovieUserRating(int $userId, int $movieId): ?setMovieUserRating{
